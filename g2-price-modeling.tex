...
SARIMA stander for Seasonal Autoregressive Integrated Moving Average\((P,D,Q)_s\)which is allow to convert ARIMA model to seasonal by account the seasonal observation ,where some periods is repeats many time in data sets for example yearly, monthly ,SARIMA model in order AR Eq.~\ref{Eq-AR} and MA Eq.~\ref{Eq-MA}  predict \(y_i\) at same seasonality periods,form historical data sets \(X\) and with seasonal as monthly where the length of data split to quarterly \(S = 4\) ,then AR would be use \(\beta_{t-4}\) and MA  \(\delta_{t-4}\), second model MA would be  \(\beta_{t-4}\)  and \(\beta_{t-8}\) and AR \(\delta_{t-4}\) and \(\delta_{t-8}\).\\
