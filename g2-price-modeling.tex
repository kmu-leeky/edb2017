...
SARIMA stander for Seasonal Autoregressive Integrated Moving Average\((P,D,Q)_s\) allow to convert ARIMA model to seasonal by account the seasonal observation ,where some seasonal \(S\) is repeats many periods in data sets for an example take historical time series data\(X\) for one year and we will predict \(y_s\) with seasonal \(s= 4\) ,then the model will split the length of time series by number of seasonal ,so we will get 4 periods each content 3 months Figure~\ref{fig:sarima} from (1 to 3) as 4th lag ,(4 to 6) 3rd lag ,(7 to 9) 2nd lag and (10 to 12) as first lag ,then the looking at same seasonal time \( (x_{0}, x_{{s-1}}, x{{s-2}}, x_{{s-3}} )\) the number of periods will chosen based on number of lag given by AR Eq.~\ref{Eq-AR} and MA Eq.~\ref{Eq-MA} if we set AR and MR coefficient as 2 then the AR would be\(\beta_{s}\) and \(\beta_{s}\) and the MA \(\delta_{s}\) and \(\delta_{s-1}\).\\
...
As it was widely known that the spot instnace price is hard to be predicted, most of recent work focused on increasing stability of applications that run on spot instances. DeepSpotCloud~\cite{deep-spot-cloud} and Flint~\cite{flint} proposed fast task migration mechanisms when a service interruption event happens. As the spot price prediction is challenging, both approaces used the naive method~\cite{how-to-bid-cloud,not-bid-cloud}, and we believe that the finding in this paper can significantly improve the cost gain and system stability of the previous approaches.
